...
FOLDED_BETA_BINOMIAL_GLM=stan_model(file="folded_beta_binomial_glm.stan", save_dso=F, auto_write=F)
...
foldedBetaBinomialGLM=function(ys,ns,xFull,xNull) {
  stopifnot(all(xNull==xFull[,1:ncol(xNull)]))
  stopifnot(all(xNull[,1]==1))

  
  dat=list(N=length(ys),P=ncol(xNull),ys=ys,ns=ns,x=xNull, concShape=1.0001, concRate=1e-4)
  
  # Fit null model
  fit_null <- optimizing(FOLDED_BETA_BINOMIAL_GLM, data=dat, algorithm="BFGS", hessian=T, as_vector=F)
  ...
  fit_full <- optimizing(FOLDED_BETA_BINOMIAL_GLM, data=datFull, init=initFull, algorithm="BFGS", hessian=T, as_vector=F)
...
null_test_foldedbinomial_glm=function() {
  ...
  anova_result=foldedBetaBinomialGLM(ys_folded,ns,xFull,xNull)
